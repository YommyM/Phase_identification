...
def plot_Voronoi(vor, sorted_regions, points, n_count, box, phase, title_str): #n_count = lip_num*2 + chol_num in one leaflet
    ...
    for i in range(len(sorted_regions)):
        ...
        if(i < len(phase)):
            ...
            ax.fill(*zip(*vertices), alpha=0.1, color='grey')     
...
for ts in u.trajectory[b:e:interval]:
    ...
    lip_area_up = dict(zip(list(range(1,91)) + list(range(181,253)) + list(range(477, 495)), lip_area_up))
    lip_area_low = dict(zip(list(range(91, 181)) + list(range(253, 325)) + list(range(495, 513)), lip_area_low))
