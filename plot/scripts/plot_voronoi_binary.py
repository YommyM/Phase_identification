...
def plot_Voronoi(ax, vor, sorted_regions, points, n_count, box, phase): #n_count = lip_num*2 + chol_num in one leaflet
    ...
    ax.plot(points[:692, 0], points[:692, 1], 'b.', markersize='2')         
    ax.plot(points[692:1152, 0], points[692:1152, 1], 'r.', markersize='2')   
    ax.plot(points[n_count:, 0], points[n_count:, 1], '.', color='white', markersize='2', alpha = 0.0)       
...
for b in range(start, end, n_gap):
    ...
    atom_phase_upper_lip = [item for item in atom_phase_fr[:576] for _ in range(2)]
    atom_phase_lower_lip = [item for item in atom_phase_fr[576:1152] for _ in range(2)]
    HMM_phase_upper_lip = [item for item in HMM_phase_fr[:576] for _ in range(2)]
    HMM_phase_lower_lip = [item for item in HMM_phase_fr[576:1152] for _ in range(2)]
